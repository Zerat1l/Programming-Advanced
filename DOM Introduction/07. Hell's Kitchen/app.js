function solve() {
   ...
   function onClick () {
      ...
      let bestResturantsSalary = 0;
      let best = undefined;

      for (const name in restaurants) {
         if(restaurants[name].averageSalary > bestResturantsSalary) {
            bestResturantsSalary = restaurants[name].averageSalary;
            best = {name, ...restaurants[name]}
         }
      }
      bestResturantsP.textContent = `Name: ${best.name} Average Salary: ${best.averageSalary.toFixed(2)} Best Salary: ${best.bestSalary.toFixed(2)}`
